...
#ifndef _PROTO_CYASSL_SOCK_H
#define _PROTO_CYASSL_SOCK_H
...
extern struct xprt_ops cyassl_sock;
int cyassl_sock_handshake(struct connection *conn, unsigned int flag);
int cyassl_sock_prepare_ctx(struct bind_conf *bind_conf, CYASSL_CTX *ctx, struct proxy *proxy);
void cyassl_sock_free_certs(struct bind_conf *bind_conf);
int cyassl_sock_prepare_all_ctx(struct bind_conf *bind_conf, struct proxy *px);
int cyassl_sock_prepare_srv_ctx(struct server *srv, struct proxy *px);
